...
function SettingsPage() {
 ...
 return crm.length >= 4 && /^\d+$/.test(crm);
};
 ...
 if (clinicEmail && !validateEmail(clinicEmail)) {
 newErrors.clinicEmail = 'Email inválido';
}

 if (clinicPhone && !validatePhone(clinicPhone)) {
 newErrors.clinicPhone = 'Formato inválido. Use: (XX) XXXXX-XXXX';
}

 if (crm && !validateCRM(crm)) {
 newErrors.crm = 'CRM deve conter apenas números (mínimo 4 dígitos)';
}
 ...
 setShowSelectPatientDialog(true);
};
 ...
 {/* Confirmation Dialog: Select Patient for Deletion */}
 <ConfirmDialog
 open={showSelectPatientDialog}
 onOpenChange={setShowSelectPatientDialog}
 title="Seleção de Paciente para Exclusão"
 description="Esta funcionalidade permite selecionar pacientes específicos para exclusão de dados. Em breve você poderá escolher quais pacientes terão seus dados removidos do sistema."
 confirmText="Entendi"
 onConfirm={() => setShowSelectPatientDialog(false)}
 variant="default"
 />
 ...
 title="⚠️ Exclusão Permanente de Conta"
 description="ATENÇÃO: Você está prestes a solicitar a exclusão PERMANENTE de sua conta. Esta ação NÃO pode ser desfeita. Todos os seus dados, pacientes e histórico serão removidos permanentemente. Deseja continuar?"
 ...
 {/* Success Dialog: Account Deletion Requested */}
 <ConfirmDialog
 open={showDeleteAccountSuccess}
 onOpenChange={setShowDeleteAccountSuccess}
 title="Solicitação Enviada com Sucesso"
 description="Nossa equipe entrará em contato em até 48 horas para confirmar a exclusão e fornecer instruções para exportação dos dados. Você receberá um email com os próximos passos."
 confirmText="Fechar"
 onConfirm={() => setShowDeleteAccountSuccess(false)}
 variant="default"
 />

 <div className="flex flex-col min-h-[600px] h-[calc(100vh-4rem)]">
...
"flex-1 h-10 transition-all font-medium min-w-[120px]",
 activeTab === tab.id
 ?'bg-gradient-to-r from-[#8C00FF] to-[#450693] text-white shadow-md'
 :'text-gray-700'
 )}
 ...
 className={cn(
 "bg-gray-50 border-gray-300 focus:ring-2 focus:ring-[#8C00FF] focus:border-transparent",
 errors.crm && "border-red-500 focus:ring-red-500"
 )}
 placeholder="123456"
 />
 {errors.crm && <p className="text-sm text-red-600 mt-1">{errors.crm}</p>}
 </div>
 <div>
 ...
 if (errors.clinicPhone) setErrors(prev => ({ ...prev, clinicPhone: '' }));
 }}
 className={cn(
 "bg-gray-50 border-gray-300 focus:ring-2 focus:ring-[#8C00FF] focus:border-transparent",
 errors.clinicPhone && "border-red-500 focus:ring-red-500"
 )}
 placeholder="(11) 99999-9999"
 />
 {errors.clinicPhone && <p className="text-sm text-red-600 mt-1">{errors.clinicPhone}</p>}
 </div>
 <div className="md:col-span-2">
 ...
 if (errors.clinicEmail) setErrors(prev => ({ ...prev, clinicEmail: '' }));
 }}
 className={cn(
 "bg-gray-50 border-gray-300 focus:ring-2 focus:ring-[#8C00FF] focus:border-transparent",
 errors.clinicEmail && "border-red-500 focus:ring-red-500"
 )}
 placeholder="contato@clinicamedica.com.br"
 />
 {errors.clinicEmail && <p className="text-sm text-red-600 mt-1">{errors.clinicEmail}</p>}
 </div>
 </div>
 </div>
 )}
